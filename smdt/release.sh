...
PROJECT=wordchip-a20/Android4.2/A20-420-V30/${PRODUCT}
...
get_source_and_build()
{
	...
	cp -adR ${BUILDDIR}/$A20_VERSION/android/out/target/product/wing-${PRODUCT}/*-ota-*.zip ${UPDATEDIR}/update.zip || exit 1;
	sync;
}
...
EOF
	...
	expect -f $BUILDDIR/upload $1 $2 || exit 1;
}
